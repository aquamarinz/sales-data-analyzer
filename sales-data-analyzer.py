...
def generate_target_completion_report(sales_data, sales_target, target_type):
    ...
    sales_sum = sales_this_month.groupby("店员")["实收"].sum()
    ...
    target_completion["已完成"] = target_completion["实收"].round().astype(int)
...
def save_target_completion_report():
    ...
    output_file = f"各档口个人实收{target_type}完成度（{today}）.xlsx"
...
def generate_rankings(sales_data, sales_target, yesterday):
    ...
    ranking3_numeric = ranking3.copy()

    for clerk in ranking3.index:
        clerk_target = sales_target[sales_target["店员"] == clerk]
        if not clerk_target.empty:
            if ranking3.loc[clerk] >= clerk_target["日高标"].values[0]: # Check for '日高标' first
                ranking3.loc[clerk] = f"{ranking3.loc[clerk]} 完成高标"
            elif ranking3.loc[clerk] >= clerk_target["日中标"].values[0]:
                ranking3.loc[clerk] = f"{ranking3.loc[clerk]} 完成中标"
    ...
    return ranking1, ranking2, ranking3, ranking4, ranking5, ranking6, ranking7, ranking3_numeric
...
def generate_team_performance(sales_data, sales_target, yesterday):
    ...
    store_sales = sales_data.groupby(["门店", "店员"])["实收"].sum().reset_index().groupby("门店").sum(numeric_only=True)
    ...
    for store in store_targets.index:
        ...
        store_sales_sum = store_sales.loc[store, "实收"]
        ...
        yesterday_sales_sum = sales_yesterday[sales_yesterday["门店"] == store]["实收"].sum()
        
        # Convert to '万' for display
        store_target_high_in_wan = store_target_high / 10000
        store_sales_sum_in_wan = store_sales_sum / 10000
        
        yesterday_target_low_in_wan = yesterday_target_low / 10000
        yesterday_sales_sum_in_wan = yesterday_sales_sum / 10000

        store_performance = (
            f"{store}\n"
            f"月度高标： {store_target_high_in_wan:.1f}万\n"
            f"已完成： {store_sales_sum_in_wan:.1f}万\n"
            f"低标完成率 {low_completion_rate:.1f}%\n"
            f"高标完成率 {high_completion_rate:.1f}%\n\n"
            f"昨日团队完成情况\n"
            f"昨日低标： {yesterday_target_low_in_wan:.1f}万\n"
            f"实际完成： {yesterday_sales_sum_in_wan:.1f}万\n"
            f"{'-' * 35}\n"
        )
...
def export_rankings_to_txt(rankings, team_performance):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        for index, (ranking, title) in enumerate(zip(rankings[:-1], ranking_titles), start=1): # Exclude ranking3_numeric from the loop
            ...
            if index == 3: # Modify the total sum calculation for 昨日个人实收排名
